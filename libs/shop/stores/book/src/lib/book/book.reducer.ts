import { createReducer, on, State, Action, createFeatureSelector } from '@ngrx/store';
...
const initBookReducer = createReducer(
  ...
  on(fromActions.loadRequestSuccess, (state, action) => ({ ...state, bookList: action.payload, isLoading: false }))
);
...
export const bookStateSelector = createFeatureSelector<BookState>(
  'bookState'
);